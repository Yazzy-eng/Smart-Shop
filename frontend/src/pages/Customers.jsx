  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div>
        ...
        {selectedCustomer && (
          <div className="print-area bg-white rounded-xl border border-slate-200 p-5 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="font-semibold text-slate-800">{selectedCustomer.name}</h2>
                <p className="text-sm text-slate-500">{selectedCustomer.phone}</p>
                <p className="text-xs text-slate-400 mt-1 print:block hidden">
                  Statement printed {new Date().toLocaleDateString()} — Deeqsan Store
                </p>
              </div>
              <button
                onClick={() => window.print()}
                className="print:hidden text-xs px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200"
              >
                Print Statement
              </button>
            </div>
            ...
            {statement && !statement.error && (
              <>
                      ...
                      <div key={p.id} className="flex justify-between items-center text-sm">
                        ...
                        <span className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="print:hidden text-red-400 hover:text-red-600 text-xs"
                          >✕</button>
                        </span>
                      </div>
                    ))}
                ...
                <form onSubmit={handleRecordPayment} className="print:hidden border-t border-slate-100 pt-4 space-y-2">
